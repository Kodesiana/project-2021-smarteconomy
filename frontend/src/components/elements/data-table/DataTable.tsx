...
import { ColumnDef, flexRender, getCoreRowModel, getFilteredRowModel, getPaginationRowModel, getSortedRowModel, TableState, Updater, useReactTable } from "@tanstack/react-table";
...
export const DataTable = ({ className, data, meta, loading, columns, state, setState, ...props }: DataTableProps) => {
  ...
  const component = useMemo(() => <div className={clsx(styles.dataTable, className)} {...props}>
    <Table
      fontSize="xs"
  ...
  </div>, [className, props, table, loading, data.length, renderImage]);
